    ...
    const handleTokenChange = () => {
      ...
      if (token) {
        try {
          ...
          setNavigation([
            { name: "Home", href: `/users/${decodedToken.userId}/decks/decklist`, current: false, icon: HomeIcon },
            { name: "Browse", href: "/browse", current: false, icon: MagnifyingGlassIcon },
            { name: "Profile", href: `/users/${decodedToken.userId}/profile`, current: false, icon: UserCircleIcon },
          ]);
        } catch (error) {
        ...
        <div className="relative flex h-16 items-center justify-between">
          ...
          <div className="flex w-[76px] items-center justify-start">
            <div className="flex shrink-0 items-center">
              ...
              />
              <span className="ml-3 text-blue-700 font-bold text-lg hidden sm:block">Recallify</span>
            </div>
          </div>
                ...
                navigation.map((item) => (
                  <a
                    ...
                    className={classNames(
                      item.current ? "bg-gray-900 text-white" : "text-black font-semibold hover:bg-blue-950 hover:text-white",
