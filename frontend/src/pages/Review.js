...
const Review = () => {
  ...
  useEffect(() => {
    const fetchCards = async () => {
      try {
        const response = await fetch(`http://localhost:8000/users/${userId}/decks/${deckId}/cards/reviewcards`);
  ...
  const handleCardResult = async (result) => {
	...
	try {
	  const response = await fetch(
		`http://localhost:8000/users/${userId}/decks/${deckId}/cards/${currentCard._id}/review`,
		{
		  method: "PUT",
		  headers: {
